...
namespace udit
{
    ...
    const string Scene::fragment_shader_code =
        ...
        "void main()"
        "{"
        "    vec3 ambient = ambient_intensity * light.color;"
        ""
        "    vec3 norm = normalize(Normal);"
        "    vec3 lightDir = normalize(vec3(light.position) - FragPos);"
        "    float diff = max(dot(norm, lightDir), 0.0);"
        "    vec3 diffuse = diffuse_intensity * diff * light.color;"
        ""
        "    vec3 viewDir = normalize(view_pos - FragPos);"
        "    vec3 halfwayDir = normalize(lightDir + viewDir);"
        "    float spec = pow(max(dot(norm, halfwayDir), 0.0), shininess);"
        "    vec3 specular = specular_intensity * spec * light.color;"
        ""
    ...
    void Scene::configure_material (GLuint program_id)
{
    ...
    GLint shininess = glGetUniformLocation(program_id, "shininess");
...
void Scene::configure_light (GLuint program_id)
{
    ...
    GLint specular_intensity = glGetUniformLocation (program_id, "specular_intensity");
    GLint view_pos = glGetUniformLocation(program_id, "view_pos");
    ...
    // Configurar la posición de la cámara (view position)
    // glm::vec3 camera_pos = camera.get_position(); // Asegúrate de obtener la posición correcta de la cámara
    glUniform3f(view_pos, 0.1f, 0.1f, 0.1f);
}
